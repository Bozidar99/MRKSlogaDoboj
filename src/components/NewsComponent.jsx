...
function NewsComponent() {
  ...
  const { vijesti, igracUtakmice, loading } = useSelector((state) => state.news)
  ...
  const vijest = sliderVijesti[trenutna] || sliderVijesti[0]

  const sljedeća = () => setTrenutna((prev) => (prev + 1) % sliderVijesti.length)
  const prethodna = () => setTrenutna((prev) => (prev - 1 + sliderVijesti.length) % sliderVijesti.length)
